    ...
    for (int j = 0; j < n - i - 1; j++) {
      if (arr[j] > arr[j + 1]) {
        ...
        for (int k = 0; k < n; k++) {
          ...
          rect.setPosition(
              sf::Vector2f(k * 12, 600 - arr[k])); // 修改为 sf::Vector2f
          if (k == rest_max_index) {
...
int DisplaySortState(std::vector<int> &arr, std::vector<int> &pivotIndexList,
                     sf::RenderWindow &window, int i, int j, int pivotIndex,
                     int pivotValue, int left, int right, int frameCount) {
  if (left >= right) {
    return frameCount;
  }
...
std::vector<RunState> SubQuickSort(std::vector<int> &arr,
                                   std::vector<int> &pivotIndexList,
                                   sf::RenderWindow &window, int left,
                                   int right, int frameCount) {
  if (right == -1) {
    right = arr.size() - 1;
  }
  if (left >= right) {
  ...
  for (int j = left; j < pivotIndex; ++j) {
    if (arr[j] < pivotValue) {
      ...
      window.clear();
      frameCount =
          DisplaySortState(arr, pivotIndexList, window, i, j, pivotIndex,
                           pivotValue, left, right, frameCount);
    }
  }
  std::this_thread::sleep_for(std::chrono::milliseconds(40));
  window.clear();
  frameCount =
      DisplaySortState(arr, pivotIndexList, window, i + 1, pivotIndex,
                       pivotIndex, pivotValue, left, right, frameCount);
  std::swap(arr[i + 1], arr[pivotIndex]);
  window.clear();
  frameCount = DisplaySortState(arr, pivotIndexList, window, i + 1, pivotIndex,
                                i + 1, pivotValue, left, right, frameCount);
...
void quickSort(std::vector<int> &arr, sf::RenderWindow &window, int left,
               int right) {
  right = arr.size() - 1;
  std::vector<int> pivotIndexList;
  ...
  while (start < end) {
    ...
    auto states = SubQuickSort(arr, pivotIndexList, std::ref(window), left_,
                               right_, frameCount);
    if (!states.empty()) {
